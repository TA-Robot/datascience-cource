...
#### 1.2.2 推定理論
- 点推定とその性質（不偏性、一致性、有効性）  
- 最尤推定（MLE）、モーメント法、ベイズ推定（事前分布・事後分布）  
- 区間推定と信頼区間の考え方

#### 1.2.3 仮説検定
- 帰無仮説・対立仮説、p値、有意水準  
- 単標本／二標本 z検定、t検定、F検定、χ²検定  
- 多重比較問題と補正（Bonferroni、Holm、FDRなど）  
- 検定力とサンプルサイズ設計

#### 1.2.4 ノンパラメトリック手法
- Wilcoxon、Mann-Whitney、Kruskal-Wallis検定など  
- ノンパラメトリックな信頼区間
...
#### 1.3.1 線形回帰の基本
- 単回帰・重回帰モデル  
- Gauss-Markov定理（BLUEの概念）  
- 回帰係数の最小二乗推定（OLS）

#### 1.3.2 回帰診断とモデル選択
- 残差分析（QQプロット、残差 vs. フィット）  
- 多重共線性とVIF（Variance Inflation Factor）  
- 過学習防止、AIC、BIC、クロスバリデーション

#### 1.3.3 分散分析（ANOVA）
- 一元配置分散分析  
- 二元配置分散分析、反復測定ANOVA  
- 多重比較法（Scheffé、Tukey、Bonferroniなど）

#### 1.3.4 カテゴリカルデータ解析
- ロジスティック回帰と対数オッズ比  
- 対数線形モデル  
- 多カテゴリ分析（多項ロジスティック、尤度比検定）
...
#### 1.4.2 主成分分析（PCA）とSVD
- 固有値分解、特異値分解の基礎  
- PCAによる次元削減と可視化  
- 高次元データへの応用

#### 1.4.3 判別分析と因子分析
- 線形判別分析（LDA）、二次判別分析（QDA）  
- 因子分析（共通因子モデル）  
- 構造方程式モデリング（SEM）の概要
